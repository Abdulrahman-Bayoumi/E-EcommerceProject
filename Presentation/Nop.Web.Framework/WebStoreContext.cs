...
public partial class WebStoreContext : IStoreContext
{
    ...
    public virtual async Task<Store> GetCurrentStoreAsync()
    {
        ...
        {
            //if database is not properly installed or stores don't exist, return null
            return null;
        }
    }
    ...
    public virtual Store GetCurrentStore()
    {
        ...
        //wrap everything in try-catch to prevent any exceptions during installation
        try
        {
            //check if database is installed
            ...
            //safely try to find store by host, if fails just get first store
            Store store = null;
            try
            {
                store = allStores.FirstOrDefault(s => _storeService.ContainsHostValue(s, host));
            }
            catch
            {
                //if ContainsHostValue fails, just get first store
            }
        ...
        {
            //if database is not properly installed or stores don't exist, return null
            //this prevents exceptions during installation process
            return null;
        }
    }
